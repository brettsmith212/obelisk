...
9. ⬜ **Vault Settings sheet (minimal).** Shows vault path, indexing status, "Re-index now". Lives behind the top-bar overflow `⋯` for now; full Settings screen is Phase F.
    - **Pair with: invert write authorization from allowlist → denylist.** Modern agent UX (Claude Code, Cursor, ChatGPT) defaults to "can write anywhere the user granted access" and lets the user *block* specific paths. Land that flip with this sheet so the deny-list editor ships alongside the inversion.
        - Replace [`VaultWriter.defaultAuthorizedFolders`](./Sources/Services/Vault/VaultWriter.swift) (`["obelisk"]`) with `defaultDenyList = [".obsidian", ".trash"]` (hard-coded, non-overridable — editing Obsidian's own config bricks the vault).
        - Add `userDenyList: [String]` sourced from `UserDefaults`, surfaced as an editable list in this sheet.
        - Flip `checkAuthorized` → `checkNotDenied` (same shape, opposite logic).
        - Drop the `ReadDailyNoteTool` folder-authorization special case — it becomes a normal `VaultWriter.write` call once the vault root is no longer "unauthorized."
        - `source: obelisk` overwrite refusal stays load-bearing: it's what keeps user-authored notes anywhere in the vault safe from the model. Atomic write stays.
        - Update [§7 "do no harm" rules](#7-the-do-no-harm-rules-for-vault-writes) and the [validation checklist](#9-validation-checklist) "unauthorized folder" line to reflect the new model.
        - **Why pair them:** flipping to denylist before the settings UI exists means the model can write anywhere with no UI for the user to lock things down. Land the inversion + editor together or not at all.
